...
if (tableDataEntry) {
  ...
  // Found zLZO at offset 1422
  const lzoOffset = 1422;
  console.log(`Trying to decompress from offset ${lzoOffset}...\n`);
  
  // Skip the "zLZO" marker
  const lzoData = bytes.slice(lzoOffset + 4);
  
  // Try to read the uncompressed size hint (usually first 4 bytes after magic)
  const view = new DataView(bytes.buffer, bytes.byteOffset + lzoOffset);
  let sizeHint = 0;
  try {
    sizeHint = view.getUint32(4, true); // offset +4 from lzoOffset
  } catch { }
  console.log(`Size hint from header: ${sizeHint}\n`);
  ...
  if (decompressed && decompressed.length > 100) {
    ...
    // Check if this contains VBScript
    if (text.includes('Sub ') || text.includes('Function ')) {
      console.log(`✓ Found VBScript (${text.length} chars)!\n`);
      ...
      console.log('Decompressed but doesn\'t look like VBScript.');
