...
// ─── Physics Init — moved to src/app/physics-init.ts ────────────────────────

// resetBall + resetGameState — moved to src/app/game-state.ts

// ─── Physics Worker Setup — moved to src/app/physics-worker-setup.ts ────────
...
if (FPW_ROLE === 'dmd') {
  ...
  (async () => {
    ...
    try {
      ...
      const handles = await initPhysics(setPhysics);
      leftFlipperColliderHandle = handles.leftFlipperColliderHandle;
      rightFlipperColliderHandle = handles.rightFlipperColliderHandle;
